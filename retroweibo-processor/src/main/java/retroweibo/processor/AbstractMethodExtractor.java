 ...
 * interface Foo ... { ... } } so that we can discover {@code @RetroWeibo} classes that are nested
 * in an interface.
 *
...
final class AbstractMethodExtractor {
  ...
  // checking that the method has no parameters, because an @RetroWeibo class will cause a compiler
  // error if there are abstract methods with parameters, since the @RetroWeibo processor doesn't
